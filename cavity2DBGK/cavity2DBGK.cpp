...
int main() {
    ...
    RegularizedVelocityBoundaryEdgeXP* velboundaryYP = new RegularizedVelocityBoundaryEdgeXP(nu, ux, uy);
    ...
    block.traversal(setOp.changeTo(velboundaryYP, 0), 1, X, Y + 1, Y + 1); //顶部边界晶格定义一个速度
	...
	for (int t = 1; t <= nstep; ++t) {
		...
		if (t % interval == 0) {
			...
			outvtk.ascii("./outfile/cavity2dbgk_" + std::to_string(t) + std::string(".vti"), 1, X, 1, Y);
		}
	}
	stop = clock();
